...
**Production / deploy:** the whole stack is containerised in `docker-compose.yml` — `caddy` (auto-TLS reverse proxy, the only Internet-facing service on :80/:443), `web` (`Dockerfile`, Next.js standalone), `cms` (`cms/Dockerfile`, non-root Strapi), `mcp` (`mcp/Dockerfile`, authenticated Strapi gateway with opt-in create/update and no delete), and `db` (Postgres with a least-privilege Strapi role). Every service has a healthcheck and bounded Docker logs. Caddy routes `olegthelilfix.me`→web and `cms.olegthelilfix.me`→cms. MCP is not routed by Caddy or public DNS: Docker publishes it only on server loopback `127.0.0.1:3001`, and clients use an SSH tunnel plus a Bearer token. Generate `.env` with `scripts/generate-production-env.sh`; create the first CMS owner and a scoped Strapi API token privately with `docker-compose.bootstrap.yml`; only then start the public stack. Full procedure, DNS and rollback are in `DEPLOY.md`.
